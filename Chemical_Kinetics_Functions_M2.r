...
generate_grid_M2 <- function(lb,ub,n_k)
...
produce_all_priors_M2 <- function(lbound,ubound,n_k)
...
{ 
  ...
  k <-  generate_grid_M2(lbound,ubound,n_k)
...
# We combine the four vectors into a matrix
   ...
   write.table(prior_k_frame, file="prior_k_M2.csv", row.names=FALSE, col.names=FALSE, append = FALSE)  
   write.table(prior_u_frame, file="prior_u_M2.csv", row.names=FALSE, col.names=FALSE, append = FALSE)  
   write.table(prior_w_frame, file="prior_w_M2.csv", row.names=FALSE, col.names=FALSE, append = FALSE)  
...
integrate_density_M2 <- function(k,density_k,lb = 0, ub=1E+99)
...
integrate_3densities_M2 <- function(densities,lb = 0, ub=1E+99)
...
{ # The argument 'densities' should be a numerical matrix with 4 columns, so that: 
  ...
  f_u_k = densities[,3] ; f_w_k = densities[,4]   
   ...
   I_k <- integrate_density_M2(k,f_k_k,lb,ub)
   I_u <- integrate_density_M2(k,f_u_k,lb,ub)
   I_w <- integrate_density_M2(k,f_w_k,lb,ub)   
...
test_produce_priors_M2 <- function(lb,ub,n_k)
# Plot the 3 priors. Returns the integrals of the 3 priors.
{
priors <-  produce_all_priors_M2(lb,ub,n_k)

int <- integrate_3densities_M2(priors) # We integrate first the three priors.
...
################
# Definition of the profile of R according to M2
# t (us)   k (/s)    R0 (mol/m^3)
...
Compute_R_profile_M2 <- function(t,R0,k){
  R = R0/(2*k*R0*t*1E-12+1)
  return (R)
}
#################

plot_profile_M2 <- function(t_model,R_model)
{
# Plot the model profile along with the measurements.
 ...
 R_profile_model = data.frame(t_model,R_model)  # We can only plot a data frame with ggplot.
 colnames(R_profile_model) <- c("t","R")      
  
 #print(  c("R_profile_model: ",R_profile_model ) )  #DEBUGGING
 
  p = ggplot() + 
    geom_point(data = R_profile_exp, aes(x = t, y = R, color = "blue")) +
    geom_line(data = R_profile_model, aes(x = t, y = R, color = "black")) +
  ...
  write.csv(R_profile_model, file = "R_M2.csv")   # Save the model predictions.
}
...
Compute_likelihood_M2 <- function(R_profile_exp,R0,k,epsilon){
...
R <- Compute_R_profile_M2(t_exp,R0,k)   # Profile predicted by k at the 
...
Compute_likelihood_all_M2 <- function(R0,lbound,ubound,n_k,epsilon)
...
{ # Compute the likelihood function between the lower bound and
 ...
 # system("rm Likelihood_M2.csv")  # The file must be empty at the beginning
                                  # of the loop ("rm" = remove).
                                  # It'll be incrementally filled. 


	fn <- "Likelihood_M2.csv"
	#Check its existence
	if (file.exists(fn)) 
  ...
  k_all =  generate_grid_M2(lbound,ubound,n_k)   # We generate the grid.
...
# print(c("n_k = ",n_k))
  ...
  # Debugging mode
  # print("Calling  ' Compute_likelihood_all_M2'  ")
  # print(" k_all:   ")
  # print( k_all)
  # print("  R_profile_exp:  ")
  # print(R_profile_exp)  
  ...
  # print(c("epsilon: ", epsilon) )  
   ...
   for(i in 1:n_k) { 
    ...
    L =  Compute_likelihood_M2(R_profile_exp,R0,k_all[i],epsilon) 
      d <- cbind(k_all[i],L)
#      print(c("i= ",i))
write.table(d, file="Likelihood_M2.csv", row.names=FALSE, col.names=FALSE, append = TRUE)  
...
Examine_likelihood_M2 <- function(R0,lbound,ubound,n_k,epsilon=0.065)
...
max_likelihood =  Compute_likelihood_all_M2(R0,lbound,ubound,n_k,epsilon)

print(c("Max lilelihood: ",max_likelihood))
  
L_all <- read.table("Likelihood_M2.csv", header=FALSE)
...
##################
Compute_posterior_M2 <- function(k,prior,L)
...
Compute_all_posteriors_M2 <- function(R0,lbound,ubound,n_k,epsilon=0.65,lb_plot,ub_plot)
...
# print(c("R0",R0,"lbound",lbound,"ubound",ubound,"n_k",n_k,"epsilon",epsilon,"lb_plot",lb_plot,"ub_plot",ub_plot)); 
  ...
  priors <-  produce_all_priors_M2(lbound,ubound,n_k)
# We begin by producing the 3 priors.     
  
  
# integrate_density_M2 <- function(k,density_k,lb = 0, ub=1E+99)  
  
int_prior <- integrate_3densities_M2(priors)
...
L_all_frame <- read.table("Likelihood_M2.csv", header=FALSE)
...
posterior_k <- Compute_posterior_M2(k_all,f_k_k,L_all)
norm_k = scan("normalisation.csv")

posterior_u <- Compute_posterior_M2(k_all,f_u_k,L_all)  
norm_u = scan("normalisation.csv")

posterior_w <- Compute_posterior_M2(k_all,f_w_k,L_all)
...
I_k <- integrate_density_M2(k_all,posterior_k)

I_u <- integrate_density_M2(k_all,posterior_u)

I_w <- integrate_density_M2(k_all,posterior_w)   
...
write.table(posterior_k_frame, file="posterior_k_M2.csv", row.names=FALSE, col.names=FALSE, append = FALSE)  
write.table(posterior_u_frame, file="posterior_u_M2.csv", row.names=FALSE, col.names=FALSE, append = FALSE)  
write.table(posterior_w_frame, file="posterior_w_M2.csv", row.names=FALSE, col.names=FALSE, append = FALSE)  
